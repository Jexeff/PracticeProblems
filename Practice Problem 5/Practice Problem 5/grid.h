...
public:
	friend class Grid<T>;
	GridIterator(Node<T>* p = NULL, bool x = false, unsigned int w = -1, unsigned int h = -1) { 
	...
	GridIterator<T>& operator++(int) {
		...
		if (spiral == false) {
			...
			if (grid_size == 1) {
				...
				grid_size--;
				if (ptr->right == NULL || ptr->right == top_right) {
					direction = "right";
					ptr = ptr->down;
					top_right = ptr;
					return *this;
				...
				grid_size--;
				if (ptr->down == NULL || ptr->down == bottom_right) {
					direction = "bottom";
					ptr = ptr->left;
					bottom_right = ptr;
					return *this;
				...
				grid_size--;
				if (ptr->left == NULL || ptr->left == bottom_left) {
					direction = "left";
					ptr = ptr->up;
					bottom_left = ptr;
					return *this;
				...
				grid_size--;
				if (ptr->up == NULL || ptr->up == top_left) {
					direction = "top";
					ptr = ptr->right;
					top_left = ptr;
					return *this;
...
public:
	...
	iterator begin_spiral() {
		return iterator(upper_left, true, width, height);
	}
