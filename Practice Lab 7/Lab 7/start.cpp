...
// constructor and two public member variables.  This is one of the
...
int findRoutes(int x, int y, vector<vector<bool>> const& blocked_grid, int total) {

    if (x == 0 && y == 0) {
        total++;
        std::cout << "Origin found." << std::endl;
        return total;
    }
    else if (blocked_grid[x][y] == true) {
        return total;
    }
    else if (x == 0) {
        findRoutes(x, y - 1, blocked_grid, total);
    }
    else if (y == 0) {
        findRoutes(x - 1, y, blocked_grid, total);
    }
    else {
        findRoutes(x - 1, y, blocked_grid, total);
        findRoutes(x, y - 1, blocked_grid, total);
...
int main(int argc, char* argv[]) {
  ...
  int total = 0;
  total = findRoutes(start_x, start_y, blocked_grid, total);
 // std::cout << "The current total is " << total << " route(s)." << std::endl;
