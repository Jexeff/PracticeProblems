...
void InsertStudentIntoSchoolPreferenceList(std::istream &istr, std::list<School> &schools) {
  ...
  for (std::list<School>::iterator it = schools.begin(); it != schools.end(); ++it) {
      if (school_name == it->GetName()) {
          name_found = true;
          it->InsertStudentName(new_student_name, insert_before_student_name);
      }
  }
  if (name_found == false) {
      std::cerr << "could not insert student into school preference list, no school named "<< school_name << std::endl;
  }
...
void RemoveSchoolFromStudentPreferenceList(std::istream& istr, std::list<Student>& students) {
    ...
    for (std::list<Student>::iterator it = students.begin(); it != students.end(); ++it) {
        if (student_name == it->GetName()) {
            it->RemoveSchoolName(school_name);
        }
    }
...
void PrintSchoolPreferenceList(std::istream &istr, const std::list<School> &schools, std::ostream &ostr) {
  ...
  // FILL IN THIS FUNCTION
  //
  bool school_found = false;
  for (std::list<School>::const_iterator it = schools.begin(); it != schools.end(); ++it) {
      if (it->GetName() == school_name) {
          school_found = true;
          ostr << school_name << " preference list:" << std::endl;
          it->PrintSchoolPreferenceList(ostr);//implement ostr
      }
  }

  if (school_found == false) {
      std::cerr << "could not print student preference list for " << school_name << ", that school does not exist" << std::endl;
  }
 }
...
void PrintStudentPreferenceList(std::istream &istr, const std::list<Student> &students, std::ostream &ostr) {
  ...
  for (std::list<Student>::const_iterator it = students.begin(); it != students.end(); ++it) {
      if (it->GetName() == student_name) {
          student_found = true;
          ostr << student_name << " preference list:" << std::endl;
          it->PrintStudentPreferenceList(ostr);  //implement ostr
      }
  }
  if (student_found == false) {
      std::cerr << "could not print school preference list for " << student_name << ", that student does not exist" << std::endl;
  }
}
